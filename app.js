...
function more(){$('#view').innerHTML=`<section class="screen">${pageHead('More')}<div class="card identity"><img src="assets/more-mau-journey.png?v=29" alt="You and Mau looking toward the mountain summit"><div><strong>Unburdened</strong><span>Less weight. More life.</span></div></div><button class="menu-card" id="photosBtn"><span>▧</span><div><strong>Progress Photos</strong><small>Front • side • back every 4 weeks</small></div><b>›</b></button><button class="menu-card" id="settingsBtn"><span>⚙</span><div><strong>Settings</strong><small>Goals and reminders</small></div><b>›</b></button><button class="menu-card" id="dataBtn"><span>⇩</span><div><strong>Your Data</strong><small>Export or restore a backup</small></div><b>›</b></button></section>`;$('#photosBtn').onclick=photosPage;$('#settingsBtn').onclick=settingsPage;$('#dataBtn').onclick=dataPage}
