...
function runSplash(){const s=$('#splash');if(!s)return;const bar=$('#splashProgress'),pct=$('#splashPercent'),duration=4000,start=performance.now();document.body.classList.add('splashing');function tick(now){const progress=Math.min(1,(now-start)/duration),n=Math.floor(progress*100);if(bar)bar.style.width=`${n}%`;if(pct)pct.textContent=`${n}%`;if(progress<1){requestAnimationFrame(tick)}else{if(bar)bar.style.width='100%';if(pct)pct.textContent='100%';s.classList.add('leave');setTimeout(()=>{s.remove();document.body.classList.remove('splashing')},650)}}requestAnimationFrame(tick)}
