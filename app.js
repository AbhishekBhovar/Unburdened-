...
function more(){$('#view').innerHTML=`<section class="screen more-screen">${pageHead('More')}<div class="card identity"><img src="assets/more-screen-journey.jpg?v=45" alt="You and Mau looking toward the mountain summit"><div><strong>Unburdened</strong><span>Less weight. More life.</span></div></div><button class="menu-card" id="settingsBtn"><span>⚙</span><div><strong>Settings</strong><small>Goals and reminders</small></div><b>›</b></button><button class="menu-card" id="dataBtn"><span>⇩</span><div><strong>Your Data</strong><small>Export or restore a backup</small></div><b>›</b></button></section>`;$('#settingsBtn').onclick=settingsPage;$('#dataBtn').onclick=dataPage}
